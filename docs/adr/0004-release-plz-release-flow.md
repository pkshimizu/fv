...
- **crates.io へは publish しない。** `Cargo.toml` に `publish = false` を設定。`fv` はライブラリではなくバイナリアプリであり、release-plz もこの設定を尊重して `cargo publish` をスキップする。`CARGO_REGISTRY_TOKEN` は不要。
...
- **`build.yml` を `release.yml` へ置換。** 旧 `build.yml`（main push のたびに 3 プラットフォームをビルドしアーティファクト化）を廃止。リリース作成時のみ macOS arm64 / macOS x86_64 / Linux x86_64 を release ビルドし、`gh release upload` で Release 資産に添付する。
