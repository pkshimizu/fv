# 2. System info: header-resident, synchronous ~5s refresh on the main tick
...
- **~5s throttle, internal to the reader.** A wall-clock throttle (`RefreshThrottle`) refreshes the dynamic fields when ≥ 5 s have elapsed since the last refresh (`Instant`-based, not a tick counter — the interval is independent of the main-loop iteration rate and input activity). Static fields are gathered once in `new()` and never re-read. CPU% accuracy needs ≥ ~200 ms between samples, which 5 s comfortably satisfies.
...
- **Refresh on every main-loop iteration (~100 ms when idle):** rejected. No reading value at that rate for a human, and it churns the render loop needlessly.

## Consequences

- The first frame shows `CPU 0%` until the first ~5 s refresh produces a real sample (CPU% needs two spaced reads). Acceptable.
